...
from clrnet.models.losses.focal_cross_entropy_loss import CombinedLoss
from clrnet.models.losses.WeightedAdaptiveFocalLoss import WeightedAdaptiveFocalLoss
from clrnet.models.losses.Asymmetricfocalloss import FocalLoss_A
from clrnet.models.losses.soft_focal_loss import SoftFocalLoss
from clrnet.models.losses.FocalLossWithLabelSmoothing import FocalLossWithLabelSmoothing
...
from clrnet.models.losses.laneiou_loss import laiou_loss 
from clrnet.models.losses.laneiou_loss_new import dynamic_liou_loss
from clrnet.models.losses.GLIoU_loss import gliou_loss
from clrnet.models.losses.G_Laiou_loss import g_laiou_loss
...
class CLRHead(nn.Module):
    def __init__(self,
                 num_points=72,   # 表示每个偏移量的数量。
                 prior_feat_channels=64,  # 先验特征通道数。
                 fc_hidden_dim=64, # 全连接层的隐藏维度
                 num_priors=192,  # 先验框的数量
                 num_fc=2,# 全连接层的数量
                 refine_layers=3,# 细化层的数量
                 sample_points=36, # 采样点的数量
                 cfg=None):
        ...
        self.seg_decoder = SegDecoder(self.img_h, self.img_w,      # 用于分割任务的解码器
    ...
    def loss(self,
        ...
        # cls_criterion = WeightedAdaptiveFocalLoss(alpha=0.25, gamma=2.)
        cls_criterion = FocalLoss_A(alpha=0.25, gamma_pos=2.,gamma_neg=2.)
        # cls_criterion = SoftFocalLoss(alpha=0.25, gamma=2.)
        # cls_criterion = FocalLossWithLabelSmoothing(alpha=0.25, gamma=2.)
        ...
        for stage in range(self.refine_layers):
            ...
            for predictions, target in zip(predictions_list, targets):
                ...
                if len(target) == 0:
                    ...
                    cls_target = predictions.new_zeros(predictions.shape[0]).long()  #  predictions.shape[0]表示 predictions 张量的第一个维度的大小 数据类型转换为 long 类型
                    cls_pred = predictions[:, :2] #  提取 predictions 中每个样本的前两个类别预测值，形成一个形状为 [batch_size, 2] 的张量 cls_pred。
                ...
                with torch.no_grad():
                    matched_row_inds, matched_col_inds = assign(
                        predictions, target, self.img_w, self.img_h, self.prior_ys,)  # 传递prior_ys
                ...
                reg_yxtl = predictions[matched_row_inds, 2:6]  # predictions 是一个形状为 [batch_size, num_classes] 的张量，其中 num_classes 是每个样本的预测值数量。
                reg_yxtl[:, 0] *= self.n_strips  #  是一个预定义的值，表示条带数（通常是图像分割中的条带数）
                reg_yxtl[:, 1] *= (self.img_w - 1)  # reg_yxtl[:, 1] 表示 reg_yxtl 中每一行的第二个值。self.img_w 是图像的宽度。 *= 操作符将 reg_yxtl 中每一行的第二个值乘以 (self.img_w - 1)。
                reg_yxtl[:, 2] *= 180
                ...
                # iou_loss = iou_loss + gliou_loss(
                #     reg_pred, reg_targets,
                #     self.img_w, length=15)

                # iou_loss = iou_loss + g_laiou_loss(
                #     reg_pred, 
                #     reg_targets,
                #     img_w=self.img_w,
                ...
                # # 修改后的调用
                # iou_loss = iou_loss + dynamic_liou_loss(
                #     reg_pred, 
                #     reg_targets,
                #     img_w=self.img_w,
                #     img_h=self.img_h,
                #     min_length=3,  # 可调整的参数
                #     max_length=20  # 可调整的参数
                # )
                ...
                # print(cls_accuracy,cls_acc_stage)
                # P1 = sum(cls_acc_stage) 
                # P2 = len(cls_acc_stage)
                # print(P1,P2)
