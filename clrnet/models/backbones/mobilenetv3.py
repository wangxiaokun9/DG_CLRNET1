...
class MobileBottleneck(nn.Module):
    def __init__(self, inp, oup, kernel, stride, exp, se=False, nl='RE', ca=False):
        super(MobileBottleneck, self).__init__()
        assert stride in [1, 2]
        assert kernel in [3, 5]
        padding = (kernel - 1) // 2
...
class MobileNetV3Wrapper(nn.Module):
    def __init__(self,
                 ...
                 in_channels=[24,40,48,96,576],
                 pretrained=True,
                 cfg=None):
        super(MobileNetV3Wrapper, self).__init__()
        self.cfg = cfg
        self.in_channels = in_channels
        self.model = eval(mobilenet)(
...
class MobileNetV3(nn.Module):
    def __init__(self, input_size=224, mode='small', width_mult=1.0):   # n_class=1000 n_class：模型的输出类别数量，默认是 1000。 input_size：输入图像的大小，默认是 224x224。 dropout=0.8 dropout：dropout 比例，默认是 0.8。 mode：模型的大小，可以是 ‘large’ 或 ‘small’，默认是 ‘small’。 width_mult：宽度乘数，用于调整模型的宽度，默认是 1.0。
        ...
        self.part2 = nn.Sequential(*self.features[4:9])
        self.part3 = nn.Sequential(*self.features[9:])
        # # # building classifier
