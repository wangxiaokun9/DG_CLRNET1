...
class EfficientNet(nn.Module):
    ...
    def __init__(self,
        ...
        part1 = inverted_residual_setting[:3]
        part2 = inverted_residual_setting[3:8]
        part3 = inverted_residual_setting[8:]

        layers1 = OrderedDict({"stem_conv": ConvBNActivation(in_planes=3,
        ...
        for cnf in part1:
            layers1.update({cnf.index: block(cnf, norm_layer)})
        for cnf in part2:
            layers2.update({cnf.index: block(cnf, norm_layer)})
        for cnf in part3:
            layers3.update({cnf.index: block(cnf, norm_layer)})
 
        self.part1 = nn.Sequential(layers1)
        self.part2 = nn.Sequential(layers2)
        self.part3 = nn.Sequential(layers3)
        ...
        # self.part1 = nn.Sequential(*self.features[1:3])
        # self.part2 = nn.Sequential(*self.features[3:5])
        # self.part3 = nn.Sequential(*self.features[5:])
...
def efficientnet_b0(num_classes=1000, pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = torch.load('./efficientnet-b0-355c32eb.pth')  
...
def efficientnet_b1(num_classes=1000, pretrained=True, **kwargs):
    ...
    if pretrained:
        # state_dict = torch.load('mobilenetv3_small_67.4.pth.tar')
        state_dict = torch.load('./efficientnet-b1-f1951068.pth')  
        # model.load_state_dict(state_dict, strict=True)
        model.load_state_dict(state_dict, False)
        # raise NotImplementedError       
        # return model
...
if __name__ == "__main__":
    ...
    from torchsummaryX import summary   # 使用 torchsummaryX 库来打印 PyTorch 模型的结构和参数
    summary(model, torch.zeros(1, 3, 224, 224))  #(batchsize,channels,img_h,img_w)
