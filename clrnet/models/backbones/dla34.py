...
class DLA(nn.Module):
    ...
    def forward(self, x):
        y = []  # 创建了一个空列表 y，用于存储每个层级的输出
        x = self.base_layer(x)   # 输入 x 通过 base_layer 进行处理
        for i in range(6):   # 这个循环从 0 到 5，依次通过 level0 到 level5 这六个层级。每个层级的输出都会被添加到列表 y 中
            x = getattr(self, 'level{}'.format(i))(x)  # 动态地获取当前层级的层对象。
            y.append(x)
        return y[2:]   # 返回从第三个层级（即 level2）开始的所有输出。y[2:] 表示从索引 2 开始到列表末尾的所有元素。
